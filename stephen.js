...
var resetballCounter =0; //stephen made death counter
...
var BeginningScreen= true; //stephen made start screen
...
function ballMovement() {
  ...
  // bottom
  if(ballY > canvas.height) {   // Will reset the ball and "use" an attempt if ball goes out of bounds
    playerAttempts--;
    ballReset();
  }
...
function drawAll() {
  ...
  if(showEndingScreen) {
    if(playerScore == maximumScore) {
      ...
      text("Death Count: " + resetballCounter, canvas.width/2, 400, 'white', 'bold 2em Arial', 'center');
  ...
  if(BeginningScreen){
    if(playerScore == 0){
      text("Click To Play", canvas.width/2,100,'white','bold 3em Arial', 'center');
      text("Move Mouse to Move", canvas.width/2, 550, 'white', 'bold 1.5em Arial', 'center');
    }
    return;
  }
  //ball
  ...
  text("Lives: " + playerAttempts, 673, 30, 'white', 'bold 1.4em monospace', 'left');
}
